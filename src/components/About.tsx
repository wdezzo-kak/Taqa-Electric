      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row-reverse gap-6 md:gap-10 items-center relative z-10">
        ...
        <Animation direction="right" delay={100} className="w-full md:w-1/2 relative">
          ...
          <Animation delay={300} direction="up">
            <div className="absolute -bottom-4 md:-bottom-6 lg:-bottom-8 -left-4 md:-left-6 lg:-left-8 bg-surface-container p-4 md:p-6 lg:p-8 border border-outline-variant/20 max-w-xs hover:shadow-glow transition-shadow duration-300">
              <span className="material-symbols-outlined text-primary text-3xl md:text-4xl lg:text-5xl mb-2 md:mb-3 lg:mb-4 block group-hover:scale-110 transition-transform duration-300">
                engineering
              </span>
              <p className="text-[10px] font-bold tracking-[0.4em] text-on-surface-variant uppercase">
                {t.techDivision}
              </p>
              <p className="text-on-surface font-headline font-bold text-base md:text-lg lg:text-xl uppercase mt-1 lg:mt-2">
                {t.certExperts}
              </p>
            </div>
          </Animation>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              {features.map((feature, index) => (
                <Animation key={index} delay={600 + (index * 100)} direction="up" className="group cursor-pointer">
                  <h5 className="text-on-surface font-headline font-bold text-xs lg:text-sm tracking-widest uppercase group-hover:text-primary transition-colors duration-300">
                    {feature.title}
                  </h5>
